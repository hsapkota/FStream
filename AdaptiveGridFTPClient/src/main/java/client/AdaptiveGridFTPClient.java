...
public class AdaptiveGridFTPClient {
    ...
    public static double startTransferTime;
    ...
    public static void main(String[] args) throws Exception {
        ...
        Thread streamThread = new Thread(() -> {
            try {
                AdaptiveGridFTPClient.startTransferTime = System.currentTimeMillis();
    ...
    private void lookForNewData() throws IOException {
        ...
        if (newDataset != null && isNewFile) {
            ...
            if ((overAllParams.getConcurrency() + channelInUse.size()) > conf.maxConcurrency){
                int tmpCC = conf.maxConcurrency - channelInUse.size();
                if(tmpCC > 1){
                    tmpCC = 1;
                }
                overAllParams.setConcurrency(tmpCC);
            }
    ...
    public void checkNewData() throws InterruptedException {
        if (dataNotChangeCounter < 1000) {
            ...
            if (ConfigurationParams.profiling && (profilingCounter > 1 &&
                    chunks != null & profilingCounter % 2 == 0)) {

                int[][] estimatedParamsForChunks = new int[chunks.size()][4];
                tuneNewChunkParameters(estimatedParamsForChunks);
                // ProfilingOps.simpleHillClimbing(conf.maxConcurrency, conf.channelDistPolicy);
                ProfilingOps.simpleHillClimbingModified(conf.maxConcurrency, conf.channelDistPolicy);
                ...
                for (FileCluster chunk : chunks) {
                    ...
                    // System.out.println("Check new Data Function called.");
                    ...
                    // System.out.println("isConcChange: "+isConcurrencyChange + ", isPipeChange: " + isPipeChange + ", newPipe: " + sessionParametersMap.get(chunk.getDensity().toString()).getPipelining() + " for chunk: " + chunk.getDensity().toString());
                    ...
                    // if(isConcurrencyChange && chunk.getDensity().toString().equals("LARGE")){
                    //     System.out.println("Printing out all pipelining for LARGE chunks and changeCC is " + sessionParametersMap.get(chunk.getDensity().toString()).getConcurrency()
                    //      + "is PipeChange: "+ isPipeChange + " and newPipe: " + sessionParametersMap.get(chunk.getDensity().toString()).getPipelining() 
                    //      + " and pipe for id=0 is " + chunk.getRecords().channels.get(0).getPipelining());
                    //     printPipeline(chunk);
                    // }
...
//        int totalChannelCount = Math.min(conf.maxConcurrency, dataset.count());
        for (int i = 0; i < estimatedParamsForChunks.length; i++) {
            ...
            chunks.get(i).getTunableParameters().setConcurrency(1); //Remove this line
...
//        historicalFirstCheck = false;
                ...
                if (otherChannels != null) {
                    for (ChannelModule.ChannelPair cp : otherChannels.keySet()) {
                        if (!otherChannels.get(cp)) {
                            ...
                            cp.pipelining = chunk.getTunableParameters().getPipelining();
                            chunk.getRecords().channels.add(cp);
                            index++;
    ...
    private void changePipeline(FileCluster chunk) {
        ...
        // Sysddtem.out.println("Pipelining changing chunk: "+ chunk.getDensity().toString());
        for (ChannelModule.ChannelPair c : chunk.getRecords().channels) {
        //    System.out.println("Cur pipie: " + c.getPipelining() + " and CC id is " + c.getId());
    ...
    private void printPipeline(FileCluster chunk){
        for (ChannelModule.ChannelPair c : chunk.getRecords().channels) {
            System.out.println("For channelId: " + c.getId() + " the pipelining value is " + c.getPipelining());
         }
    }
