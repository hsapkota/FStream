...
//        }
    ...
    public static void allocateChannelsToChunks(List<FileCluster> chunks, final int channelCount,
        ...
        System.out.println("Allocating channels to " + chunks.size() + " chunks");
        ...
        if (channelDistPolicy == ConfigurationParams.ChannelDistributionPolicy.ROUND_ROBIN) {
            ...
            int count = 0;
            for (int i = 0; count < channelCount && count < totalFileCount; i++) {
                int index = i % modulo;
                ...
                // Recalculate total file count in case some files are 
                // being tranferred in the meantime
                totalFileCount = 0;
                for (int j = 0; j < chunks.size(); j++) {
                    fileCount[j] = chunks.get(j).getRecords().count();
                    totalFileCount += fileCount[j];
                }
            }
            ...
            for (int i = 0; i < totalChunks; i++) {
                ...
                System.out.println("Chunk " + fileCluster.getDensity().toString() + " : " + concurrencyLevels[i] + " channels");
            }
        } else {
            ...
            for (int i = 0; i < totalChunks; i++) {
                FileCluster fileCluster = chunks.get(i);
                fileCluster.getTunableParameters().setConcurrency(concurrencyLevels[i]);
                System.out.println("Chunk " + fileCluster.getDensity() + " weight " + chunkWeights[i] + " cc: " + concurrencyLevels[i]);
            }
        }
    }
